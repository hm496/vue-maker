...
let webpack = null;
let config = null;
let formatWebpackMessages = null;
let printHostingInstructions = null;
let printBuildError = null;
...
function getSrcHash () {
  return getFilesHash(appDirectory, [path.relative(appDirectory, paths.appBuild) + "/**"]).then(function (hash) {
    ...
    // 获取 .srchash
    try {
      oldSrcHash = fs.readFileSync(
        path.join(paths.appBuild, '.srchash')
      )
...
function nextStep () {
  ...
  printHostingInstructions = require('react-dev-utils/printHostingInstructions');
